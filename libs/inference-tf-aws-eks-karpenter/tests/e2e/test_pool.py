"""Gated live E2E — `jd pool` over the heterogeneous pool set (MNG + Karpenter).
...
blind to the subsystem backing each pool:
  - jd pool list   — lists all pools by name (managed node group AND Karpenter NodePools)
...
Two fundamentally different wirings are unified here:
  ...
  - **Karpenter NodePools** (`cpu`, `gpu-g`, `gpu-p`) — read via the k8s custom API
...
Marked `full_deployment` — reads the live base deployment as-is (no GPU, non-mutating).
"""
...
KARPENTER_POOL = "cpu"  # Karpenter NodePool (k8s custom API) — always-on, not GPU-gated

# Every Karpenter NodePool the base config provisions (cpu + both GPU pools). gpu-p is
# gated on enable_gpu_p_nodepool, which the default preset turns on.
EXPECTED_KARPENTER_NODEPOOLS = {"cpu", "gpu-g", "gpu-p"}
...
def test_pool_list_includes_managed_and_karpenter_pools(
...
@pytest.mark.full_deployment
def test_pool_list_places_managed_nodegroup_first(
    e2e_deployment: EndToEndDeployment,
    kubernetes_cluster_login: None,
) -> None:
...
@pytest.mark.full_deployment
def test_pool_list_json_contains_all_pools(
    e2e_deployment: EndToEndDeployment,
    kubernetes_cluster_login: None,
) -> None:
...
def test_pool_status_ready_for_both_wirings(
    e2e_deployment: EndToEndDeployment,
    kubernetes_cluster_login: None,
    pool_name: str,
) -> None:
...
@pytest.mark.full_deployment
@pytest.mark.parametrize("pool_name", HAPPY_PATH_POOLS)
def test_pool_show_json_name_matches(
    e2e_deployment: EndToEndDeployment,
    kubernetes_cluster_login: None,
    pool_name: str,
) -> None:
...
@pytest.mark.full_deployment
def test_pool_show_karpenter_resource_shape(
    e2e_deployment: EndToEndDeployment,
    kubernetes_cluster_login: None,
) -> None:
...
@pytest.mark.full_deployment
def test_pool_show_managed_nodegroup_resource_shape(
    e2e_deployment: EndToEndDeployment,
    kubernetes_cluster_login: None,
) -> None:
...
@pytest.mark.full_deployment
def test_pool_status_not_found(
    e2e_deployment: EndToEndDeployment,
    kubernetes_cluster_login: None,
) -> None:
