"""Shared cluster/node helpers for e2e and load tests.

Generic node- and pod-level operations run via `kubectl debug node` / `kubectl` — reusable by
any test that needs to inspect a node's host, time a pull, or roll a node's containerd config in
place. Nothing here is feature- or benchmark-specific.
"""
...
def node_debug(node: str, image: str, script: str) -> str:
    ...
    res = run_kubectl(
        "debug", f"node/{node}", "-q", f"--image={image}", "--",
        "chroot", "/host", "sh", "-c", script, check=False,
    )
    return res.stdout
...
def write_containerd_dropin(node: str, image: str, path: str, toml_body: str) -> None:
    ...
    """
    # A random-ish sentinel keeps the outer heredoc delimiter from colliding with body content.
    script = (
...
def remove_containerd_dropin(node: str, image: str, path: str) -> None:
    ...
    node_debug(node, image, f"rm -f {path}; systemctl restart containerd 2>/dev/null; true")
...
def node_of_pod(namespace: str, pod: str) -> str:
    """The node a pod landed on (polls briefly until scheduled)."""
    for _ in range(30):
        node = run_kubectl(
